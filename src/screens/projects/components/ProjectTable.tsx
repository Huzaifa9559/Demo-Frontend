import { Space, Table, Tag, Typography, type TablePaginationConfig, type TableProps } from 'antd'
  ...
  const columns: TableProps<ProjectRecord>['columns'] = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (_, record) => (
          <Space>
            <Button size="small" type="link" onClick={() => openDetails(record)}>
              View
            </Button>
            <Button size="small" type="text" onClick={() => openEditForm(record)}>
              Edit
            </Button>
          </Space>
        ),
